...
// Запит на отримання постів
const { data: response, pending, refresh } = await useFetch('http://127.0.0.1:8000/api/admin/blog/posts', {
    query: { page }
})

// Безпечне отримання даних (додано || для уникнення помилок, якщо даних ще немає)
    ...
    { accessorKey: 'title', header: 'Заголовок' },
    {
        ...
        cell: ({ row }: { row: any }) => h(
            UDropdownMenu,
            {
                items: [
                    [
                        // Виправлено синтаксис шляхів (додані зворотні лапки)
