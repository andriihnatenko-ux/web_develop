<template>
    <div class="max-w-4xl mx-auto py-10 px-4">
        <UCard class="w-full">
            <template #header>
                <h1 class="text-2xl font-bold">Редагувати пост</h1>
            </template>
            ...
            <UForm :schema="schema" :state="state" class="space-y-6" @submit="onSubmit">
                ...
                <div class="flex justify-end gap-4 pt-4 border-t border-gray-800">
                    ...
                    <UButton type="submit" color="primary" size="lg" :loading="pending">Оновити пост</UButton>
                </div>
            </UForm>
        </UCard>
    </div>
...
const { data: postData } = await useFetch<any>(`${apiBase}/posts/${postId}`)

if (postData.value?.data) {
    const p = postData.value.data
    state.title = p.title
    state.category_id = p.category_id || undefined
    state.content_raw = p.content_raw || undefined
    state.is_published = p.is_published || false
}
